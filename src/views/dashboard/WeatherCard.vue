    ...
    <VCardText>
      <div class="d-flex align-end mb-7">
        <div class="d-flex gap-3 align-end pb-1">
          <VIcon icon="ri-shining-fill" size="14" />
          <VIcon icon="ri-sun-line" size="14" />
          <VIcon icon="ri-cloud-line" size="14" />
          <VIcon icon="ri-moon-fill" size="14" />
        </div>
        <div class="flex-fill text-right align-end">
          <h4 class="text-h3 hotness">{{ weather }} <span class="text-h6">&deg;C</span></h4>
        </div>
      </div>
      <div class="mb-8">
        <div class="d-flex justify-space-between my-3 degrees">
          <div v-for="degree in degreesProgres">{{ degree }}&deg;C</div>
        </div>
        <v-progress-linear
          v-model="barValue"
          height="4"
      ...
      <VRow class="match-height">
        <VCol cols="12" sm="4" xs="6" class="pa-1" v-for="item in weatherItems">
